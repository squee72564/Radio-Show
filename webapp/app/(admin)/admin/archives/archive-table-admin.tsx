...
export const columns = (onDelete: (id: string) => void): ColumnDef<ArchiveRowData>[] => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="truncate flex items-center justify-center text-center">
          {<LocalDate date={date} />}
        </div>
      );
    },
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-row justify-center">
          <DeleteArchiveComponent archiveId={archiveId} onDelete={onDelete} />
        </div>
      );
    },
  },
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="destructive" disabled={pending || submissionstate?.type === 'success'}>
          {!submissionstate ? 'Delete' : submissionstate.type === 'success' ? 'Deleted' : 'Error'}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="flex flex-col justify-center items-center text-center gap-2">
        <p>Confirm Delete?</p>
        <p>This cannot be undone.</p>
        <Button onClick={deleteArchive} disabled={pending || submissionstate?.type === 'success'}>
          Confirm
        </Button>
      </PopoverContent>
    </Popover>
