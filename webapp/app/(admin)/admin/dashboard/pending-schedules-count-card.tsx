...
export default async function PendingSchedulesCard() {
  ...
  return (
    <Card className={pendingCount > 0 ? 'border-yellow-400/60' : 'border-green-400/50'}>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">Pending Schedules</CardTitle>
      </CardHeader>
      <CardContent>
        {pendingCount > 0 ? (
          <div className="space-y-2">
            <p>
              There are <strong>{pendingCount}</strong> schedule(s) awaiting review.
            </p>
          ...
          <p className="text-green-600/80">
            <strong>{pendingCount}</strong> pending schedules
          </p>
        )}
      </CardContent>
    </Card>
