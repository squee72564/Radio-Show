...
export default function ScheduleManagementCard({
  ...
  const [submissionstate, SetSubmissionState] = useState<Result<{ message: string }>>({
    type: 'error',
    message: '',
  });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-bold text-lg">{stream.title}</CardTitle>
      ...
      <CardContent className="min-w-0 truncate">{stream.description}</CardContent>
