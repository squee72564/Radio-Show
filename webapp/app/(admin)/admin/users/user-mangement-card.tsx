    ...
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription className="flex flex-col gap-2">
          ...
          <div className="flex flex-row gap-2">
            <p>Created at:</p> <LocalDate date={user.createdAt} />
          </div>
          {user.emailVerified ? (
            <Badge variant={'outline'}>
              Email Verified: <LocalDate date={user.emailVerified} />
            </Badge>
          ) : (
      ...
      <CardFooter className="flex flex-row justify-center items-center gap-5">
        {!isUserAdmin(user.status) && (
          <Button variant="secondary" disabled={isPending || disabled} onClick={handleMakeAdmin}>
            {isPending ? 'Promoting...' : 'Make Admin'}
          </Button>
        )}
        {user.status === $Enums.Role.ADMIN && isOwnerViewing && (
          <Button variant="secondary" disabled={isPending || disabled} onClick={handleDemotion}>
