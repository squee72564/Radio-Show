...
export default async function ArchivePlayer({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="flex flex-col min-h-screen w-full p-6 gap-6">
      ...
      <div className="flex flex-col flex-1 gap-4 w-full">
        <CustomPlayer
          streamUrl={`/api/archive?archiveId=${streamArchive.id}`}
          isStreamLive={true}
          showControls={true}
        />
        <Alert variant="default" className="w-full">
          <AlertTitle>
            {streamArchive.streamSchedule.title} - {streamArchive.user.name}
          </AlertTitle>
          <AlertDescription className="text-base">
            <p className="">{streamArchive.streamSchedule.description}</p>
          </AlertDescription>
        </Alert>
      </div>
    </main>
